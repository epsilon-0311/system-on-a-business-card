...
/* player position */
uint8_t st_player_pos_x;
uint8_t st_player_pos_y;
...
uint8_t st_GetMissleMask(uint8_t objnr)
{
    ...
    return st_object_types[o->ot].missle_mask;
}
...
{
    st_obj *o = st_GetObj(objnr);
    return st_object_types[o->ot].hit_mask;
}
...
{
  st_obj *o = st_GetObj(objnr);
  st_player_points += st_object_types[o->ot].points;
  o->ot = 0;
}
...
{
  st_obj *o = st_GetObj(objnr);
  switch(st_object_types[o->ot].move_fn)
  {
    case ST_MOVE_NONE:
      break;
    case ST_MOVE_X_SLOW:
    ...
    case ST_MOVE_PX_FAST:
      ...
      break;
    case ST_MOVE_PLAYER:
      o->x = st_player_pos_x<<ST_FP;
      o->y = st_player_pos_y<<ST_FP;
      break;
    case ST_MOVE_PY:
      o->y += 3*ST_FP;
      break;
...
{
  st_obj *o = st_GetObj(objnr);
  switch(st_object_types[o->ot].draw_fn)
  {
    case ST_DRAW_NONE:
      break;
    case ST_DRAW_BBOX:
...
void st_Destroy(uint8_t objnr)
{
  int8_t nr;
  st_obj *o = st_GetObj(objnr);
  switch(st_object_types[o->ot].destroy_fn)
  {
...
uint8_t st_IsHit(uint8_t objnr, uint8_t x, uint8_t y, uint8_t missle_mask)
{
  ...
  switch(st_object_types[o->ot].is_hit_fn)
  {
    case ST_IS_HIT_NONE:
      break;
    case ST_IS_HIT_BBOX:
...
void st_Fire(uint8_t objnr)
{
  ...
  switch(st_object_types[o->ot].fire_fn)
  {
    case ST_FIRE_NONE: 
      break;
...
void st_Draw(uint8_t fps)
{
  switch(st_state)
  {
    ...
    case ST_STATE_IPREPARE:
      u8g2_SetFont(st_u8g2, u8g2_font_10x20_tr);
      u8g2_SetDrawColor(st_u8g2, 1);
      
      u8g2_DrawStr(st_u8g2, 0, 16, "SpaceTrash");
      u8g2_DrawHLine(st_u8g2, st_u8g2->width-st_to_diff_cnt-10, 25, 11);
      

      u8g2_SetFont(st_u8g2, u8g2_font_6x13_tr);

      u8g2_DrawStr(st_u8g2, 0, u8g_height_minus_one - (st_u8g2->height-6)/2 +10, "Highscore");
      u8g2_DrawStr(st_u8g2, 75, u8g_height_minus_one - (st_u8g2->height-6)/2 +10, st_itoa(st_highscore));
      
      u8g2_DrawStr(st_u8g2, 32, u8g_height_minus_one , "Press START");
      break;
    case ST_STATE_GAME:
      st_DrawInGame(fps);
      break;
...
/*
  player_pos: 0..255
*/
void st_StepInGame(uint8_t player_pos_x, uint8_t player_pos_y, uint8_t is_auto_fire,  uint8_t is_fire)
{
  uint8_t i, j;
  uint8_t missle_mask;
  
  /* rescale player pos */
  //st_player_pos_y = ((uint16_t)player_pos * (uint16_t)ST_AREA_HEIGHT)/256;
  
  st_player_pos_x=player_pos_x+1;
  if ( player_pos_y < 64 ){
    st_player_pos_y = 0;
  }else if ( player_pos_y >= 192 ){
    st_player_pos_y = ST_AREA_HEIGHT-2-1;
  }else{ 
    st_player_pos_y = ((uint16_t)((player_pos_y-64)) * (uint16_t)(ST_AREA_HEIGHT-2))/128;
  }
  st_player_pos_y+=1;
...
void st_Step(uint8_t player_pos_x, uint8_t player_pos_y, uint8_t start_button, uint8_t is_auto_fire, uint8_t is_fire)
{
  switch(st_state)
  {
    case ST_STATE_PREPARE:
    ...
    case ST_STATE_IPREPARE:
      ...
      if ( st_to_diff_cnt == 0 )
      {
        st_to_diff_cnt = st_u8g2->width-10;	
      }
      ...
      break;
    case ST_STATE_GAME:
      st_StepInGame(player_pos_x, player_pos_y, is_auto_fire, is_fire);
      break;
    case ST_STATE_END:
      st_to_diff_cnt = st_u8g2->width-10;		/* reuse st_to_diff_cnt */
...
void loop(void) {
  ...
  for(;;)
  {
    st_Step(x, y, !digitalRead(pin_start),/* is_auto_fire */ 0, /* is_fire */ digitalRead(pin_fire));
    u8g2.firstPage();
    do
    {
      st_Draw(0);
    ...
    if ( !digitalRead(pin_up) && y < 192 ) { // 
      y++;
    }
    
    if ( !digitalRead(pin_down) && y>=64 ) { // 
      y--;
    }
    
    if ( !digitalRead(pin_right) && x < 120 ) { // 
      x++;
    }
    
    if ( !digitalRead(pin_left) && x>6 ) { // 
      x--;
    }

    if(st_state == ST_STATE_END)
    {
      x = 6;
      y = 128;
    }
